shp2dataframe <- function(path=NULL,prj=NULL){
  require(ggmap)
  require(rgdal)
  require(foreign)
...
# ggmap goes ou to different map servers (like Google, or Open Street Map) and grabs base maps (which are raster objects) and lets you plot things on those maps
...
avl <- get_map(location="Asheville",maptype="watercolor",source = "google", zoom = 15)
ggmap(avl)
...
head(read.dbf(file = 'ggmap/CountyBoundaries/VTCountyBoundaries.dbf'))

# readOGR is from rgdal package--turns your data in a usable spatial vector object.
. <- readOGR(dsn = 'ggmap/CountyBoundaries/VTCountyBoundaries.dbf', layer = "VTCountyBoundaries")
plot(.)
...
VTcountyMap <- VTMap+geom_polygon(data = geodf, 
...
GPS_Coords <- read.table("ggmap/GPS_coords.csv",
                         header = TRUE,sep = ",",
