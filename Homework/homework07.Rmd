...
set.seed(100)
# read in data
siteData <-read.csv("Mcmillan&Noe_2013_data_site.csv", stringsAsFactors=FALSE, comment.char="#")
View(siteData)
trtData <- read.csv("Mcmillan&Noe_2013_data_treatment.csv",stringsAsFactors=FALSE, comment.char="#")
View(trtData)
```
## Assigment Objectives

1. I am examinining the factors that influence flux of water soluble phosphorus to and from inundated sediments. My hypothesis is that sites with higher rates of sedimentation 
...
ExpDesignMatrix <- matrix(ncol=)
# nReps,N
                        

# Objective 1:
# determine the effect size needed to have significant result
# with 10 observations per treatment level
while (p >= alpha){
  tE <- tE + 0.2
  for (i in seq(1,100)){
